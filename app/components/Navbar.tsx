...
const navLinks: NavLink[] = [
    ...
    { name: "Services", href: "", submenu: ["Repair", "Maintenance", "Detailing"] },
                    ...
                    {navLinks.map((item) => (
                        <li key={item.name} className="relative group">
                            <Link href={item.href} className={navItemClass}>
                                {item.name} {item.submenu && <HiChevronDown className="ml-1 w-4 h-4" />}
                            </Link>
                                    ...
                                    <ul className="pl-4 mt-1 flex flex-col gap-1">
                                        {item.submenu.map((subItem) => (
                                            <li key={subItem} className="py-1">
                                                <Link href="#" className="block text-black hover:text-secondary">
                                                    {subItem}
                                                </Link>
                                            </li>
                                        ))}
